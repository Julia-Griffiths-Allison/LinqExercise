...
namespace LinqExercise
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            //FirstName starts with a C OR an S and order this in acesnding order by FirstName.
            employees.Where(x => x.FullName.ToLower().StartsWith('c')
            ||x.FullName.ToLower().StartsWith('s'))
            .OrderBy(x => x.FirstName).ToList().ForEach(x => Console.WriteLine(x.FullName));
            ...
            employees = employees.Append(new Employee("Julia", "Griffiths Allison", 22, 1)).ToList();
